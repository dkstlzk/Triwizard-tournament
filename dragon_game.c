...
void player1_movement(Vector2 dir) {
    if (lost || won) {
        return;
    }
...
bool hitobst(Vector2 obstpos) {
    obstNode* temp = obstacles;
    while (temp != NULL) {
        if ((int)obstpos.x == (int)temp->pos.x && (int)obstpos.y == (int)temp->pos.y) {
            return true;
        }
        temp = temp->next;
    }
...
int dragon_game() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (!lost && !won) {
            ...
            if (frameCounter % 90 == 0 && frameCounter > 120){
                makefireball();
            }
            shootfireball();
            dragon_movement();
